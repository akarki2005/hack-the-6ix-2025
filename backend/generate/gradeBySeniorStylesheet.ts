...
export async function gradeBySeniorStyleSheet(
  ...
  try {
    ...
    const criteriaList = seniorCriteria
      .map(
        (criterion, index) => `
...
${
  seniorStyleSheet.exemplars
    ? Object.entries(seniorStyleSheet.exemplars)
        .slice(0, 2)
        .map(
          ([key, example]) => `
...
- Average Function LOC: ${
      seniorStyleSheet.quantitative?.avgFunctionLoc || "N/A"
    }
- Tests Added: ${seniorStyleSheet.quantitative?.testsAdded || "N/A"}
- Error Helper Usage: ${
      seniorStyleSheet.quantitative?.errorHelperUsage || "N/A"
    }
- Validation Coverage: ${
      seniorStyleSheet.quantitative?.validationCoveragePct || "N/A"
    }%
...
**Instructions:**
1. Evaluate the code changes against ALL senior-level standards listed above. If the standard mentions that no clear standard was found, briefly mention that there is nothing to comment on, and it looks fine.
...
`;
    ...
    try {
      ...
      const scores: CriterionScore[] = seniorCriteria.map(
        (criterion, index) => {
          const result =
            parsed.find((p) => p.name === criterion.name) || parsed[index];
      ...
      return seniorCriteria.map((criterion) => ({
        name: criterion.name,
        score: 70,
        justification: `LLM response could not be parsed: ${response.slice(
          0,
          200
        )}...`,
    ...
    return seniorCriteria.map((criterion) => ({
      name: criterion.name,
      score: 60,
      justification: `Error evaluating senior criteria: ${
        error instanceof Error ? error.message : "Unknown error"
      }`,
      suggestions: [
        "Senior-level manual review required due to evaluation error",
      ],
      weight: criterion.weight,
    }));
  }
